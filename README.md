...
## Два поля, які вирішують, де проєкт видно
...
| `origin` | `client` · `product` · `study` · `practice` | Чи це справжня робота. **На вітрину йдуть тільки `client` і `product`.** Навчальне й проби лишаються на дошці, але публічно не показуються. |
...
Зʼявиться картка-чернетка зі статусом `idea` і `origin: practice` — тобто поза вітриною,
поки ти не підвищиш її руками. Далі в `data/projects.json`:
`title`, `tagline`, `story`, `kind`, `origin`, `tags`, `status`.
